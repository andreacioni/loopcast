...
async function doBrowse(controlURL, objectId, browseFlag, startIndex, count) {
  const body = soapEnvelope(objectId, browseFlag, startIndex, count);
  ...
  const browseResponse = parsed["s:Envelope"]?.["s:Body"]?.["u:BrowseResponse"];
...
/**
 * Fetch metadata for a single object (container or item) rather than its
 * children -- used to resolve a folder's own title when we only have its
 * id (e.g. an item's parent, derived from item_id.split("$")).
 */
async function getMetadata(controlURL, objectId) {
  const { containers, items } = await doBrowse(
    controlURL,
    objectId,
    "BrowseMetadata",
    0,
  ...
  return containers[0] || items[0] || null;
}
...
function normalizeItem(i) {
  ...
  const protocolInfo =
    typeof res === "object" ? res["@_protocolInfo"] : undefined;
...
module.exports = { browse, getMetadata };
