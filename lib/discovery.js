...
function setJoinAllowed(enabled, durationMs = 60000) {
  if (enabled) {
    joinExpiresAt = Date.now() + durationMs;
  } else {
    joinExpiresAt = 0;
  }
...
function isJoinAllowed() {
  return Date.now() < joinExpiresAt;
}

function getJoinStatus() {
  const remainingMs = Math.max(0, joinExpiresAt - Date.now());
  return {
 ...
 *
 * When join mode is disabled, newly discovered devices are ignored and
 * not saved to disk or added to the registry.
 */
async function discover(timeoutMs = 4000) {
  ...
  runningScan = (async () => {
    try {
      ...
      const joinAllowed = isJoinAllowed();

      for (const device of seen) {
        if (registry.has(device.usn)) {
          const prev = registry.get(device.usn);
          const entry = {
            ...prev,
            ...device,
            status: "online",
