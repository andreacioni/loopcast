...
function startPolling(entry) {
  ...
  entry.pollTimer = setInterval(() => {
    entry.client.getPosition((err, seconds) => {
      if (err || entry.currentItemId == null) {
        console.error(
          `Failed to get position for item ${entry.currentItemId}: ${err}`,
        );
        return;
      }
      entry.client.getDuration((durErr, duration) => {
        if (durErr || entry.currentItemId == null) {
          console.error(
            `Failed to get duration for item ${entry.currentItemId}: ${durErr}`,
          );
          return;
        }
        saveResume(
  ...
  return new Promise((resolve) => {
    ...
    const check = () => {
      client.getTransportInfo((err, info) => {
        if (!err && info && info.CurrentTransportState === "PLAYING")
          return resolve(true);
  ...
  return new Promise((resolve) => {
    client.getTransportInfo((tiErr, transportInfo) => {
      client.callAction(
        ...
        (miErr, mediaInfo) => {
          resolve({
            transportState: tiErr
              ? `error: ${tiErr.message}`
              : transportInfo?.CurrentTransportState,
            transportStatus: tiErr
              ? undefined
...
async function play(
  rendererDesc,
  { itemId, title, mediaUrl, mimeType, dlnaFeatures, mediaKind, resumeSeconds },
) {
  console.log(
...
function status(rendererDesc) {
  return new Promise((resolve, reject) => {
    ...
    entry.client.getPosition((posErr, position) => {
      entry.client.getDuration((durErr, duration) => {
        const resumeRow = entry.currentItemId
          ? getResume(entry.currentItemId)
          : null;
        resolve({
